...
function copyRemainsDataToTempSheet(remainsSheet) {
  ...
  if (remainsData) {
    ...
    Sheets.Spreadsheets.Values.update(
      ...
      REMAINS_TECH_SS_ID,
      `${techRemainsSheetName}!R${REMAINS_TECH_DATA_FIRST_ROW}C${REMAINS_TECH_DATA_FIRST_COL}:R${remainsDataFixed.length + REMAINS_TECH_DATA_FIRST_ROW - 1}C${REMAINS_TECH_DATA_LAST_COL}`,
      {
...
function exportSheetAsXLStoDrive(spreadsheetId, sheetId, name, destinationFolderId) {
  let response = UrlFetchApp.fetch(
    `https://docs.google.com/spreadsheets/d/${spreadsheetId}/export?gid=${sheetId}`,
    {
